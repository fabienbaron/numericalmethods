...
N1=10000
t=zeros(N)
t[1:N1] = (tn - t0)/2 * (1 + cos(collect(1:N1)*pi/N1)); #t *= 0.1;
clf();
imshow(ackley_map)
tight_layout();
for i = 2:N
    stepscale=0.4#0.001+5*exp(-i/1000);
    ...
    println("iteration: $i  acceptance: $a stepscale: $stepscale")
    if (rand() < a) # Accept
...
#    plot((x[i-1:i]+5)*99+1,(y[i-1:i]+5)*99+1);
    #readline();
end
