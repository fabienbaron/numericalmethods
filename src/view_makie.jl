...
function imview(x;figtitle="",zoom=1, color=:grays)
    if zoom < 1
        zoom = 1
    end
    nx = size(x,1)
    ...
    scene = heatmap(x[center_x-zoom_span_x+1:center_x+zoom_span_x, center_y-zoom_span_y+1:center_y+zoom_span_y], colormap=color, interpolate=false);
    sc_t = title(scene, figtitle)
end
#
